    ...
    const resetPassword = async (e) => {
		...
		if (!email || !isEmailValid(email)) setError('Provide a valid email')
			...
			const { data, error } = await supabase.auth.resetPasswordForEmail(email, {
				redirectTo: new URL('/api/auth/callback?callback=/new-password', window.location.origin)
			})
                ...
                <div className="py-12 sm:px-6 lg:px-8">
                    ...
                    <div className="mx-auto w-full max-w-xs sm:max-w-sm md:max-w-lg lg:max-w-2xl">
                        <div className="border-clip">
                            <form onSubmit={resetPassword} className="bg-black rounded-3xl border-2 border-teal-300 clip-corner px-6 py-24 sm:px-12 space-y-12">
                                {success ? (
                                    <>
                                        <h1 className="text-center text-3xl font-bold leading-9 tracking-tight text-neutral-50">Check your email</h1>
                                        <h2 className="text-center text-md font-normal text-neutral-300">We sent a password reset link to {email}.</h2>
                                    </>
                                ) : (
                                    <>
